...
export class WeatherApiService {  
  ...
  getCurrentCondition(locationId: string) {    
    ...
    else this.url = `http://${environment.apis.apiHost}/currentconditions/v1/${locationId}?apikey=${environment.apis.apiKey}&details=true`
  ...
  getLocations(filter: string): Observable<any[]> {    
    ...
    else this.url = `http://${environment.apis.apiHost}/locations/v1/cities/autocomplete?apikey=${environment.apis.apiKey}&q=${filter}`
  ...
  getForecast(locationId: string) {
    ...
    else this.url = `http://${environment.apis.apiHost}/forecasts/v1/daily/5day/${locationId}?apikey=${environment.apis.apiKey}&details=true&metric=true`
